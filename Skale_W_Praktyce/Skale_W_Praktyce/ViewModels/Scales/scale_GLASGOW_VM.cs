...
namespace Skale_W_Praktyce.ViewModels
{
    internal class Scale_GLASGOW_VM : INotifyPropertyChanged
    {
        ...
        private async Task SetBookmarkImage()
        {
            ...
            if (bookmarks.Any(x => x.UserID == Settings.CurrentUserID && x.ScaleName.Contains("Glasgow")))
            {
        ...
        private async Task BookmarkScaleMethodAsync()
        {
            ...
            if (userBookmarks.Any(x => x.ScaleName.Contains("Glasgow")))
            {
                Bookmark x = await App.Database.GetBookmarkAsyncUserIDAndName(Settings.CurrentUserID, "Glasgow");
            ...
            {
                await App.Database.SaveBookmarkAsync(new Bookmark { ScaleName = "Glasgow", UserID = Settings.CurrentUserID });
