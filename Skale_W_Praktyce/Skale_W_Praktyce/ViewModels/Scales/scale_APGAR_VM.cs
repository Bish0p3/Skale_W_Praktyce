...
namespace Skale_W_Praktyce.ViewModels
{
    internal class Scale_APGAR_VM : INotifyPropertyChanged
    {
        ...
        private async Task SetBookmarkImage()
        {
            ...
            if (bookmarks.Any(x => x.UserID == Settings.CurrentUserID && x.ScaleName.Contains("APGAR")))
            {
        ...
        private async Task BookmarkScaleMethodAsync()
        {
            ...
            if (userBookmarks.Any(x => x.ScaleName.Contains("APGAR")))
            {
                Bookmark x = await App.Database.GetBookmarkAsyncUserIDAndName(Settings.CurrentUserID, "APGAR");
            ...
            {
                await App.Database.SaveBookmarkAsync(new Bookmark { ScaleName = "APGAR", UserID = Settings.CurrentUserID });
