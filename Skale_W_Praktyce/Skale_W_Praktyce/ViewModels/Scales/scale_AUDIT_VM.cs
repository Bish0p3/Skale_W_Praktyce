...
namespace Skale_W_Praktyce.ViewModels
{
    internal class Scale_AUDIT_VM : INotifyPropertyChanged
    {
        ...
        private async Task SetBookmarkImage()
        {
            ...
            if (bookmarks.Any(x => x.UserID == Settings.CurrentUserID && x.ScaleName.Contains("AUDIT")))
            {
        ...
        private async Task BookmarkScaleMethodAsync()
        {
            ...
            if (userBookmarks.Any(x => x.ScaleName.Contains("AUDIT")))
            {
                Bookmark x = await App.Database.GetBookmarkAsyncUserIDAndName(Settings.CurrentUserID, "AUDIT");
            ...
            {
                await App.Database.SaveBookmarkAsync(new Bookmark { ScaleName = "AUDIT", UserID = Settings.CurrentUserID });
